...
//  upheap_recursive(heap.size() - 1);
    return newest;
  }

  // recursive upheap
  protected int upheap_recursive(int j)
  { 
	  int p = parent(j);
      if (compare(heap.get(j), heap.get(p)) >= 0) return j; // heap property verified
      else
      {
    	  swap(j, p);
          return upheap_recursive(p);
      }
  }
  //
